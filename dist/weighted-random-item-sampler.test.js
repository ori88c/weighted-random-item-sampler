...
const weighted_random_item_sampler_1 = require("./weighted-random-item-sampler");
describe('WeightedRandomItemSampler tests', () => {
    describe('Happy path tests', () => {
        test('items should be sampled with proportion to their weight', async () => {
            ...
            // Act.
            const sampler = new weighted_random_item_sampler_1.WeightedRandomItemSampler(items, respectiveWeights);
    ...
    describe('Negative path tests', () => {
        test('constructor should throw when it is given 0 items', () => {
            expect(() => new weighted_random_item_sampler_1.WeightedRandomItemSampler([], [5, 4])).toThrow();
        });
        test('constructor should throw when the amount of items is not equal to the amount of weights', () => {
            expect(() => new weighted_random_item_sampler_1.WeightedRandomItemSampler(["item1", "item2"], [1, 2, 3])).toThrow();
            expect(() => new weighted_random_item_sampler_1.WeightedRandomItemSampler(["item1", "item2"], [51])).toThrow();
        });
        test('constructor should throw when non-positive weight is provided', () => {
            expect(() => new weighted_random_item_sampler_1.WeightedRandomItemSampler(["item1", "item2"], [1, -2])).toThrow();
            expect(() => new weighted_random_item_sampler_1.WeightedRandomItemSampler(["item1", "item2"], [0, 5.6])).toThrow();
            expect(() => new weighted_random_item_sampler_1.WeightedRandomItemSampler(["item1", "item2", "item3"], [0.3, 4, 0])).toThrow();
            expect(() => new weighted_random_item_sampler_1.WeightedRandomItemSampler(["item1", "item2", "item3"], [6, 4, -0.7])).toThrow();
        });
    });
});
//# sourceMappingURL=weighted-random-item-sampler.test.js.map